...
class OpenidsettingsAction extends SettingsAction
{
    ...
    function showContent()
    {
        ...
        $this->elementStart('ul', 'form_data');
        $this->showPrivacySettings($this, $user);
        $this->elementEnd('ul');
        $this->elementEnd('fieldset');
    ...
    static function showPrivacySettings($out, $user) {
        $prefs = User_openid_prefs::staticGet('user_id', $user->id);

        $out->elementStart('li');
        $out->checkBox('hide_profile_link', "Hide OpenID links from my profile", !empty($prefs) && $prefs->hide_profile_link);
        $out->element('input', array('type' => 'submit',
