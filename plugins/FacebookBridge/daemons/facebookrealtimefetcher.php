 ...
 */
class FacebookRealtimeFetcher extends ParallelizingDaemon
{
    /**
     *  Constructor
     *
    ...
    function getObjects()
    {
        ...
        while ($flink->fetch()) {
            ...
            if (($flink->noticesync & FOREIGN_NOTICE_RECV) ==
                FOREIGN_NOTICE_RECV) {
                $flinks[] = clone($flink);
                common_log(LOG_INFO, "FACEBOOK RTFETCH sync: foreign id $flink->foreign_id");
            } else {
    ...
    function getRealtimeUpdates($flink)
    {
        if (empty($flink)) {
            common_log(LOG_WARNING, $this->name() . " - Can't retrieve Foreign_link");
            return null;
        }

		$importer = new FacebookImport($flink);
		try {
			$realtime = FacebookRealtime::nextRealtimeUpdate($flink->foreign_id); //, strtotime($flink->last_noticesync));
        	common_debug("FACEBOOK User '{$realtime->uid}' has an update for '{$realtime->changed_fields}' until '{$realtime->time}'");
			foreach ( explode(',', $realtime->changed_fields) as $field ) {
				if (empty($field)) {
					break;	// actually just a safeguard due to bad changed_fields input
				}
				$args = array('until'=>$realtime->time+1);
				$importer->importUpdates($field, $args);
			}
			$realtime->delete();
		} catch (Exception $e) {
			common_debug("FACEBOOK getRealtimeUpdates error for {$flink->foreign_id}: ".$e->getMessage());
			if (time()-strtotime($flink->last_noticesync) < 300) {
				// Do not retrieve /home yet
				return null;
			}
		}

		$importer->importUpdates('home');
...
$fetcher = new FacebookRealtimeFetcher($id, 60, 2, $debug);
$fetcher->runOnce();
